...
public class EnchantersShotgun extends ShotgunItem implements ICasterTool, ISpellModifierItem {
    private int bulletCount;
    StatsModifier statsModifier;
    boolean creativeOnly = false;

    public EnchantersShotgun(Properties properties, int bonusDamage, double damageMultiplier, int fireDelay, double inaccuracy, int enchantability, int bullets, float spellCostMultiplier) {
        super(properties, bonusDamage, damageMultiplier, fireDelay, inaccuracy, enchantability,bullets);
        this.bulletCount = bullets;//too lazy to get bullets correctly
        statsModifier = new StatsModifier(spellCostMultiplier);

    }
    public EnchantersShotgun(Properties properties, int bonusDamage, double damageMultiplier, int fireDelay, double inaccuracy, int enchantability, int bullets, StatsModifier statsModifier) {
        super(properties, bonusDamage, damageMultiplier, fireDelay, inaccuracy, enchantability,bullets);
        this.bulletCount = bullets;//too lazy to get bullets correctly
    ...
    protected void shoot(Level world, Player player, ItemStack gun, ItemStack ammo, IBullet bulletItem, boolean bulletFree) {
        for(int i = 0; i < this.bulletCount; ++i) {
            doShoot(world, player, gun, ammo, bulletItem, bulletFree);
        }
    }
